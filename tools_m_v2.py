...
NT_CODONS=["AAA","AAG","AAC","AAT","AGA","AGG","AGC","AGT","ACA","ACG","ACC","ACT","ATA","ATG","ATC","ATT","GAA","GAG","GAC","GAT","GGA","GGG","GGC","GGT","GCA","GCG","GCC","GCT","GTA","GTG","GTC","GTT","CAA","CAG","CAC","CAT","CGA","CGG","CGC","CGT","CCA","CCG","CCC","CCT","CTA","CTG","CTC","CTT","TAA","TAG","TAC","TAT","TGA","TGG","TGC","TGT","TCA","TCG","TCC","TCT","TTA","TTG","TTC","TTT"]


def expand_aa_wildcard_motif(motif):
	expanded=[""]
	for aa in motif:
		if aa=="X":
			expanded=[prefix+letter for prefix in expanded for letter in AA_MOTIFS]
		else:
			expanded=[prefix+aa for prefix in expanded]
	return expanded


def resolve_posavg_motifs(motif_input,kind):
	kind_int=int(kind)
	...
	raw_motifs=[motif.strip() for motif in motif_input.split(",") if motif.strip()!=""]
	resolved_motifs=[]
	seen=set()

	for motif in raw_motifs:
		motif_upper=motif.upper()
		if kind_int==1 and "X" in motif_upper:
			compute_scores=True
			for expanded in expand_aa_wildcard_motif(motif_upper):
				if expanded not in seen:
					seen.add(expanded)
					resolved_motifs.append(expanded)
		else:
			motif_resolved=motif_upper if kind_int==1 else motif
			if motif_resolved not in seen:
				seen.add(motif_resolved)
				resolved_motifs.append(motif_resolved)

	return resolved_motifs,compute_scores
...
def posavg_m_wrapper(txtinfile):
	...
	include_positive_counts_per_dataset=int(variables.get("positivecountcols","0"))
	
	
	
	inputfiles=list(variables["picklenames"].split(","))
	
	motifs,compute_scores=resolve_posavg_motifs(variables["motif"],variables["kind"])
	dataset_labels=[re.split("[./]",picklefile)[-3] for picklefile in inputfiles]
	pause_scores_by_motif={}
	bkndwindow=int(variables["bkndwindow"])
	for motif in motifs:
		pause_scores_by_motif[motif]={}
		
	
	# Do score and average data.
	results_by_pickle={}
	counts_by_pickle={}
	positive_density_counts_by_pickle={}
	for picklefile in inputfiles:
		file_start_time=time.time()
		results_by_pickle[picklefile],counts_by_pickle[picklefile],positive_density_counts_by_pickle[picklefile]=posavg_m_multi(
			variables["endmode"],
			picklefile,
			variables["outfile"],
			motifs,
			...
			variables["shift"],
			return_counts=True,
			return_positive_density_counts=include_positive_counts_per_dataset,
		)
		elapsed_seconds=time.time()-file_start_time
		print("Finished analyzing "+picklefile+" in "+str(round(elapsed_seconds,2))+" seconds.")
	...
	for motif in motifs:
		for picklefile in inputfiles:
			...
			if compute_scores:	
				...
				if denom==0:
					pause_scores_by_motif[motif][dataset_label]=float('nan')
				else:
					pause_scores_by_motif[motif][dataset_label]=num/denom
	...
	if compute_scores:		
		...
		writer = csv.writer(writerfile)
		if include_positive_counts_per_dataset:
			observed_count_headers=[dataset_label+"_observed_density_gt0" for dataset_label in dataset_labels]
			writer.writerow(["motif","instances"]+observed_count_headers+dataset_labels)
		else:
			writer.writerow(["motif","instances"]+dataset_labels)
		for motif in motifs:
			total_instances=0
			observed_counts_row=[]
			if include_positive_counts_per_dataset:
				for picklefile in inputfiles:
					observed_counts_row.append(positive_density_counts_by_pickle[picklefile].get(motif,0))
			for picklefile in inputfiles:
				motif_instances=counts_by_pickle[picklefile].get(motif,0)
				total_instances+=motif_instances
			row=[motif,total_instances]
			if include_positive_counts_per_dataset:
				row+=observed_counts_row
			for dataset_label in dataset_labels:
				row.append(pause_scores_by_motif[motif].get(dataset_label,float('nan')))
			writer.writerow(row)
		writerfile.close()
...
def posavg_m_multi(endmode,picklefile,outfile,motifs,kind,frame,bkndwindowthresh,bkndwindow,ORFnorm,UTRmode,shift,footprints=None,return_counts=False,return_positive_density_counts=False):
	...
	# Build lookup tables so most motifs can be collected with a single position-driven scan.
	# Keep first/last nucleotide motifs separate because they intentionally use special semantics.
	normal_motif_lookup={}
	firstlast_metas=[]
	...
	if return_positive_density_counts:
		positive_density_counts_by_motif={meta["raw"]:0 for meta in motif_meta}
	else:
		positive_density_counts_by_motif={}
	...
	for frame in framelist:
		...
		for gene in footprints.keys():
			...
			if kind==0:
				...
				# Fast path: scan each motif length once per gene, then update all matching motifs.
						...
						if token in motif_lookup:
							...
							if normfactor is not None:
								for meta in motif_lookup[token]:
									...
									if return_positive_density_counts and counts[i]>0:
										positive_density_counts_by_motif[meta["raw"]]+=1
						i+=motiflen

				# Preserve the existing first/last motif behavior.
				...
				counts_frame=counts[frame:]		# Note that this loss here of a base or two on the end eliminates around 100 genes.
				genesequence_aa=str(Seq.Seq(genesequence[frame:]).translate())
				genelen_seq=len(genesequence_aa)
				genelen=len(counts_frame)
				# Fast path: scan each amino-acid motif length once per gene.
				for motiflen,motif_lookup in normal_motif_lookup.items():
					i=0
						...
						if token in motif_lookup:
							...
							if normfactor is not None:
								for meta in motif_lookup[token]:
									...
									counts_by_motif[meta["raw"]]+=1
									if return_positive_density_counts and counts_frame[position_nt]>0:
										positive_density_counts_by_motif[meta["raw"]]+=1
						i+=motiflen
	...
	for meta in motif_meta:
		...
		if count==0:
			print("Warning 0 count for motif "+motif_raw)
			averages[motif_raw]=[float('nan') for x in range(2*bkndwindow)]
			continue
	...
	if return_counts:
		return(averages,counts_by_motif,positive_density_counts_by_motif)
	return(averages)
